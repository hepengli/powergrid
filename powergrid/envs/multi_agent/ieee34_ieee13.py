...
from powergrid.envs.multi_agent.base import GridEnv, NetworkedGridEnv
...
class MultiAgentMicrogrids(NetworkedGridEnv):
    ...
    def _build_net(self):
        ...
        dso.add_dataset(read_data("train", 'AVA', 'NP15', '0096WD_7_N001'))
        self.dso = dso

        mg1 = GridEnv(IEEE13Bus('MG1'), load_scale=load_scale, base_power=3)
        mg1_ess1 = ESS('ESS1', bus='Bus 645', min_p_mw=-0.5, max_p_mw=0.5, capacity=2, max_e_mwh=2, min_e_mwh=0.2)
        ...
        mg1.add_dataset(read_data("train", 'AVA', 'NP15', '0096WD_7_N001'))
        net = mg1.add_to(net, 'DSO Bus 822')

        mg2 = GridEnv(IEEE13Bus('MG2'), load_scale=load_scale, base_power=3)
        mg2_ess1 = ESS('ESS1', bus='Bus 645', min_p_mw=-0.5, max_p_mw=0.5, capacity=2, max_e_mwh=2, min_e_mwh=0.2)
        ...
        mg2.add_dataset(read_data("train", 'AVA', 'NP15', '0096WD_7_N001'))
        net = mg2.add_to(net, 'DSO Bus 848')

        mg3 = GridEnv(IEEE13Bus('MG3'), load_scale=load_scale, base_power=3)
        mg3_ess1 = ESS('ESS1', 'Bus 645', min_p_mw=-0.5, max_p_mw=0.5, capacity=2, max_e_mwh=2, min_e_mwh=0.2)
...
if __name__ == '__main__':
    from powergrid.envs.multi_agent.ieee34_ieee13 import MultiAgentMicrogrids
    env_config = {
    "train": True,
    "penalty": 10,
    "share_reward": True,
    ...
    obs, info = env.reset()
    for i in range(23):
        actions = {k: v.sample() for k, v in env.action_space.items()}
