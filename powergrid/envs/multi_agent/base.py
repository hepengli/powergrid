...
class NetworkedGridEnv(MultiAgentEnv):
    ...
    def step(self, action_n):
        ...
        terminateds.update({name: done for name in self.agents})
        truncateds.update({name: done for name in self.agents})
        if (self._t % self.max_episode_steps) > 15:
            terminateds.update({"MG3": True})
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.train:
            ...
            for agent in self.agents.values():
                agent.reset(self.net, self.max_episode_steps)
        else:
            if hasattr(self, '_t'):
                self._day += 1
            else:
    ...
    def _init_space(self):
        ...
        self.action_space = ac_spaces
        self.observation_space = ob_spaces
